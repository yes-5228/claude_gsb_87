...
// CountForDashboard 按给定条件统计验收记录数量（供看板使用，与 List 同口径）。
func (s *Service) CountForDashboard(ctx context.Context, query ListQuery) (int64, error) {
	total, err := s.repo.Count(ctx, query)
	if err != nil {
		return 0, httpx.WrapInternal("统计验收记录数量失败", err)
	}
	return total, nil
}

// CountGroupedForDashboard 按列分组统计验收记录数量（供看板使用，与 List 同口径）。
func (s *Service) CountGroupedForDashboard(ctx context.Context, query ListQuery, column string) (map[string]int64, error) {
	...
	if err != nil {
		return nil, httpx.WrapInternal("统计验收分组数量失败", err)
	}
	return result, nil
}
